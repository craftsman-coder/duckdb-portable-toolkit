...
def load_orders() -> pd.DataFrame:
    ...
    """).fetchdf()
    # Make sure order_date is a proper pandas datetime
...
# --- Build the mask safely ---
mask = df["region"].isin(region) & (df["amount"] >= min_amount)

if isinstance(date_range, (tuple, list)) and len(date_range) == 2:
    start_date = pd.to_datetime(date_range[0])
    end_date = pd.to_datetime(date_range[1])
    mask = mask & (df["order_date"] >= start_date) & (df["order_date"] <= end_date)
...
daily = (
    filtered.groupby("order_date", as_index=False)["amount"]
    .sum()
